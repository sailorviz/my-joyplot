...
export function triggerPlottingTimelineForAlbums(containerRef, albumInfos){
  ...
  clusterElsArray.forEach(el => {
    clusterElsMap[el.dataset.album] = el;
  });
  ...
  const albumReleased = Object.entries(albumInfos)
    .map(([album, info]) => ({
      album,
      release_year: new Date(info.release_year, 0, 1)
    }))
    .filter(d => !isNaN(d.release_year));

  // ✅ 计算时间范围
  const domain = d3.extent(albumReleased, d => d.release_year);
  ...
  ticks.unshift(new Date(1978, 0, 1)); // 添加左端点
