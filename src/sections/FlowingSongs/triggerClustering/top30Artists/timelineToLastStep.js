export function timelineToLastStep(containerRef) {
  console.log("timelineToLastStep is triggering...")

  const clusterElsNodeList = containerRef.current.querySelectorAll(".clusterForArtist");
  ...
  clusters.forEach((clusterEl) => {
    ...
    const left = parseFloat(clusterEl.dataset.initialLeft);
    const top = parseFloat(clusterEl.dataset.initialTop);
    clusterEl.dataset.tx = 0;
    clusterEl.dataset.ty = 0;
    const tx = clusterEl.dataset.tx;
    const ty = clusterEl.dataset.ty;
    
    clusterEl.style.transition = "transform 1s ease";
    clusterEl.style.transform =  `translate(${tx},${ty}) scale(1)`;
    clusterEl.style.left = `${left}px`;
    clusterEl.style.top = `${top}px`;

    clusterEl.dataset.state = "initial";

    // 🟩 重新绑定初始 hover（使用保存的 handler 引用）
    if (clusterEl._handlers && clusterEl._handlers.initial) {
      const hi = clusterEl._handlers.initial;
  ...
  // 移除 timeline SVG
  const oldTimeline = containerRef.current.querySelector(".timeline");
  if (oldTimeline) oldTimeline.remove();
  // 移除 timeline title&legend SVG
  const oldTitleLegend = containerRef.current.querySelector(".timelineTitle");
