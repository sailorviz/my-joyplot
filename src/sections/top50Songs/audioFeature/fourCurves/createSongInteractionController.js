...
// export function createSongInteractionController({ tooltip, kdeCharts, language = 'en' }) {
...
export function createSongInteractionController({ 
  tooltip, 
  kdeCharts, 
  language = 'en',
  initialVolume = 0.5
  ...
  let currentAudio = null;
  let currentVolume = initialVolume;  // ✅ 新增：缓存当前音量
  ...
  function onClick(div) {
    ...
    if (audioSrc) {
      currentAudio = new Audio(audioSrc);
      currentAudio.volume = currentVolume;  // ✅ 新增：设置初始音量
      currentAudio.play();
      div.classList.add("playing");
      interactionState.playingDiv = div;
