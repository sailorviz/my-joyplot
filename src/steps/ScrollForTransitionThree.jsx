...
export default function ScrollForTransitionThree({ onContinue, onGoBack }){
  ...
  useEffect(() => {
    ...
    const url = `/data/text/${language}/transitionThree.md`;
    fetch(url)
      .then((res) => res.text())
      .then((text) => {
        const blocks = text
  ...
  useEffect(() => {
    ...
    scrollerRef.current
      .setup({
        step: ".trigger-step-transition-three", // 文字底部 + 后续触发点
        offset: 0.5,
        debug: false
      })
  ...
  return(
    <div className="transition-three full-screen">
      ...
      <div className="trigger-step-transition-three"></div>
      <div className="trigger-step-transition-three"></div>
      <div className="trigger-step-transition-three"></div>
    </div>
  );
