...
export default function ScrollForTransitionFour({ onContinue, onGoBack }){
  ...
  useEffect(() => {
    ...
    const url = `/data/text/${language}/transitionFour.md`;
    fetch(url)
      .then((res) => res.text())
      .then((text) => {
        const blocks = text
  ...
  useEffect(() => {
    ...
    scrollerRef.current
      .setup({
        step: ".trigger-step-transition-four", // 文字底部 + 后续触发点
        offset: 0.5,
        debug: false
      })
  ...
  return(
    <div className="transition-four full-screen">
      ...
      <div className="trigger-step-transition-four"></div>
      <div className="trigger-step-transition-four"></div>
    </div>
  );
}